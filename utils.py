...
def get_vertex_group(context, name):
    """Get a reference to the named vertex group of the active object, creating it if necessary."""
    ob = context.view_layer.objects.active  # Use view_layer to get the active object
    if ob is None:
        return None

    # Ensure the object is in 'OBJECT' mode before modifying vertex groups
    if ob.mode != 'OBJECT':
        bpy.ops.object.mode_set(mode='OBJECT')

    # Check if the vertex group exists, return it if found
    if name in ob.vertex_groups:
        return ob.vertex_groups[name]
    else:
        # Add a new vertex group if it doesn't exist
        bpy.ops.object.vertex_group_add()
        vg = ob.vertex_groups.active
